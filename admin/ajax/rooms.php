<?php 
    ...
    if(isset($_POST['add_room'])) 
    {
        ...
        if($stmt =mysqli_prepare($conn,$q2))
        {
            ...
            $flag =0;
            die('query cannot be preared - insert');
        }
    ...
    if(isset($_POST['remove_room']))
    {
        ...
        $res2 = select("DELETE FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');
        $res3 = select("DELETE FROM `room_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
        $res4 = select("DELETE FROM `room_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
        $res5 = select("UPDATE `rooms` SET `removed`=? WHERE `id`=?",[1,$frm_data['room_id']],'ii');
